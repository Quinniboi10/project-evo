# Runs preliminary commands, then a single timed command and rewards lower time per run across N runs
...
# Pre-run commands like compiling, which should not be included in the time
PREFLIGHT_COMMANDS: list[list[str]] = [["make"], ["cargo", "build", "-r"]]
...
def evaluate(workspace: Path) -> tuple[bool, float]:
    try:
        for command in PREFLIGHT_COMMANDS:
            run_and_time_cmd(command, workspace, None)
